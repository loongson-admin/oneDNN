...
void jit_lasx_1x1_convolution_fwd_t::execute_forward(
...
void jit_lasx_1x1_convolution_fwd_t::execute_forward_thr(const int ithr,
    ...
    auto rp = rtus_driver_t<lasx>::call_params_t();
...
void jit_lasx_1x1_convolution_bwd_data_t::execute_backward_data(
        const exec_ctx_t &ctx) const {
...
status_t jit_lasx_1x1_convolution_bwd_weights_t::init(engine_t *engine) {
...
void jit_lasx_1x1_convolution_bwd_weights_t::execute_backward_weights(
        const exec_ctx_t &ctx) const {
    auto diff_dst = CTX_IN_MEM(const data_t *, DNNL_ARG_DIFF_DST);
    auto src = CTX_IN_MEM(const data_t *, DNNL_ARG_SRC);
