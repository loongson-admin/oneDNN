...
status_t jit_uni_resampling_fwd_t::get_proper_kernel_for_lasx(
    ...
    if (is_src_i8 || is_dst_i8)
        return safe_ptr_assign(kernel_,
                new jit_uni_resampling_kernel_t<lasx, Xbyak_loongarch64::VReg>(conf, dst_md));
*/
    return safe_ptr_assign(kernel_,
            new jit_uni_resampling_kernel_t<lasx, Xbyak_loongarch64::XVReg>(conf, dst_md));
}
...
status_t jit_uni_resampling_fwd_t::init(engine_t *engine) {
    ...
    if (conf.isa == lasx)
        CHECK(get_proper_kernel_for_lasx(dst_md, conf));
    else {
