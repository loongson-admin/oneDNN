...
#ifndef CPU_LOONGARCH64_JIT_LASX_CONVOLUTION_HPP
#define CPU_LOONGARCH64_JIT_LASX_CONVOLUTION_HPP
...
struct jit_lasx_convolution_fwd_t : public primitive_t {
    struct pd_t : public cpu_convolution_fwd_pd_t {
        pd_t(const convolution_desc_t *adesc, const primitive_attr_t *attr,
                const typename pd_t::base_class *hint_fwd_pd)
            : cpu_convolution_fwd_pd_t(adesc, attr, hint_fwd_pd), jcp_() {}

        DECLARE_COMMON_PD_T(JIT_IMPL_NAME_HELPER("jit:", lasx, ""),
                jit_lasx_convolution_fwd_t);
        ...
        status_t init(engine_t *engine) {
            ...
            status_t status = jit_lasx_conv_fwd_kernel_f32::init_conf(
                    jcp_, *desc(), src_md(), weights_md(), dst_md(), *attr());
            if (status != status::success) return status;

            auto scratchpad = scratchpad_registry().registrar();
            jit_lasx_conv_fwd_kernel_f32::init_scratchpad(scratchpad, jcp_);
    ...
    jit_lasx_convolution_fwd_t(const pd_t *apd) : primitive_t(apd) {}

    typedef typename prec_traits<data_type::f32>::type data_t;

    status_t init(engine_t *engine) override {
        CHECK(safe_ptr_assign(kernel_,
                new jit_lasx_conv_fwd_kernel_f32(
...
private:
    ...
    std::unique_ptr<jit_lasx_conv_fwd_kernel_f32> kernel_;
};

struct jit_lasx_convolution_bwd_data_t : public primitive_t {
    struct pd_t : public cpu_convolution_bwd_data_pd_t {
        ...
        DECLARE_COMMON_PD_T(JIT_IMPL_NAME_HELPER("jit:", lasx, ""),
                jit_lasx_convolution_bwd_data_t);
        ...
        status_t init(engine_t *engine) {
            ...
            status_t status = jit_lasx_conv_bwd_data_kernel_f32::init_conf(jcp_,
                    *desc(), *diff_src_md(), *weights_md(), *diff_dst_md());
            if (status != status::success) return status;

            auto scratchpad = scratchpad_registry().registrar();
            jit_lasx_conv_bwd_data_kernel_f32::init_scratchpad(
                    scratchpad, jcp_);
    ...
    jit_lasx_convolution_bwd_data_t(const pd_t *apd) : primitive_t(apd) {}

    typedef typename prec_traits<data_type::f32>::type data_t;

    status_t init(engine_t *engine) override {
        CHECK(safe_ptr_assign(
                kernel_, new jit_lasx_conv_bwd_data_kernel_f32(pd()->jcp_)));
        return kernel_->create_kernel();
    }
...
private:
    ...
    std::unique_ptr<jit_lasx_conv_bwd_data_kernel_f32> kernel_;
};

struct jit_lasx_convolution_bwd_weights_t : public primitive_t {
    struct pd_t : public cpu_convolution_bwd_weights_pd_t {
        ...
        DECLARE_COMMON_PD_T(JIT_IMPL_NAME_HELPER("jit:", lasx, ""),
                jit_lasx_convolution_bwd_weights_t);
        ...
        status_t init(engine_t *engine) {
            ...
            status_t status = jit_lasx_conv_bwd_weights_kernel_f32::init_conf(
            ...
            auto scratchpad = scratchpad_registry().registrar();
            jit_lasx_conv_bwd_weights_kernel_f32::init_scratchpad(
                    scratchpad, jcp_);
    ...
    jit_lasx_convolution_bwd_weights_t(const pd_t *apd) : primitive_t(apd) {}

    typedef typename prec_traits<data_type::f32>::type data_t;

    status_t init(engine_t *engine) override {
        CHECK(safe_ptr_assign(
                kernel_, new jit_lasx_conv_bwd_weights_kernel_f32(pd()->jcp_)));
...
private:
    ...
    std::unique_ptr<jit_lasx_conv_bwd_weights_kernel_f32> kernel_;
