...
void jit_lasx_convolution_fwd_t::execute_forward(const exec_ctx_t &ctx) const {
...
void jit_lasx_convolution_bwd_data_t::execute_backward_data(
...
void jit_lasx_convolution_bwd_weights_t::execute_backward_weights(
        const exec_ctx_t &ctx) const {
    auto diff_dst = CTX_IN_MEM(const data_t *, DNNL_ARG_DIFF_DST);
    auto src = CTX_IN_MEM(const data_t *, DNNL_ARG_SRC);
