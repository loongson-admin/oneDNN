...
struct rhs_arg_static_params_t {
    ...
    Xbyak_loongarch64::XReg rhs_addr_reg;
    Xbyak_loongarch64::XReg rhs_helper_reg;
    bool preserve_gpr_helpers;
    bool preserve_vmm_helper;
    std::size_t abi_param_offset;
    memory_desc_wrapper dst_d;
    std::size_t tail_size;
    Xbyak_loongarch64::XReg tail_opmask;
    bool use_exact_tail_scalar_bcast;
    Xbyak_loongarch64::XReg reg_tail_size;
    bool is_tail;
...
struct static_params_t {
    static_params_t(const Xbyak_loongarch64::XReg &param1,
            const bcast_set_t &supported_strategy_set,
            const rhs_arg_static_params_t &rhs_arg_static_params);
    static_params_t(const Xbyak_loongarch64::XReg &param1,
            const rhs_arg_static_params_t &rhs_arg_static_params);

    Xbyak_loongarch64::XReg param1;
    const bcast_set_t supported_strategy_set;
...
struct rhs_arg_dynamic_params_t {
    std::map<int, Xbyak_loongarch64::Address> vmm_idx_to_out_elem_off_addr;
    std::map<int, int> vmm_idx_to_out_elem_off_val;
    std::map<int, Xbyak_loongarch64::XReg> vmm_idx_to_out_off_oprnd;

    std::map<int, Xbyak_loongarch64::Address> vmm_idx_to_oc_elem_off_addr;
    std::map<int, int> vmm_idx_to_oc_elem_off_val;
    std::map<int, Xbyak_loongarch64::XReg> vmm_idx_to_oc_off_oprnd;

    std::map<int, Xbyak_loongarch64::Address> vmm_idx_to_sp_elem_off_addr;
    std::map<int, int> vmm_idx_to_sp_elem_off_val;
    std::map<int, Xbyak_loongarch64::XReg> vmm_idx_to_sp_off_oprnd;
...
private:
     ...
     * ptr to it.
     */
    Xbyak_loongarch64::Address prepare_rhs_arg_addr(std::size_t vmm_idx,
            std::size_t rhs_arg_idx, const dnnl_post_ops::entry_t &post_op,
    ...
    void append_offset_from_operand(
            const std::map<int, Xbyak_loongarch64::XReg> &vmm_idx_to_elem_addr_off,
            int vmm_idx, const Xbyak_loongarch64::XReg &addr_reg,
            const Xbyak_loongarch64::XReg &tmp_reg, std::size_t elem_size_bytes) const;
    ...
    void execute_broadcast(const dnnl_data_type_t &data_type,
            const Vmm &tmp_reg, const Xbyak_loongarch64::Address &rhs_addr,
    ...
    void execute_broadcast_tail(const dnnl_data_type_t &data_type,
            const Vmm &tmp_reg, const Xbyak_loongarch64::Address &rhs_addr) const;
    ...
    void load_rhs_tail_statically(const dnnl_data_type_t &data_type,
            const Vmm &tmp_vmm, const Xbyak_loongarch64::Address &rhs_addr) const;
    ...
    Xbyak_loongarch64::Address remove_bcast_bit(const Xbyak_loongarch64::Address &rhs_addr) const;

    jit_generator *host_;
    const rhs_arg_static_params_t rhs_arg_static_params_;
    const Xbyak_loongarch64::XReg param1_;
